...
void show_tcp_flags(const struct tcphdr *tcp, int cr);
...
void tcp(const u_char* packet){
	...
	if (arg_v == 1){
		styled_print("bold", "TCP ",0);
		show_tcp_flags(tcp, 0);
		show_tcp_protocol(tcp,packet);
	}else if (arg_v == 2){
		styled_print("bold", "TCP",1);
		show_tcp_ports(tcp);
	}else{
		styled_print("bold", "TCP",1);
		show_tcp_ports(tcp);
...
void show_tcp_options(const u_char* packet, int size_options){
	...
	styled_print("underline", "\t\t\tOptions TCP:",1);
	...
	while(count <= size_options){
		...
		switch(type){
			...
			default:
				...
				printf("\t\t\t\tLongueur: %d\n", length);
				printf("\t\t\t\tValeur: ");
				/* On boucle sur le champ de valeur pour toutes les afficher. -2 parce que le champ length compte aussi pour le type et la longueur */
